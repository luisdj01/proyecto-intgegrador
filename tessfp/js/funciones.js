$(document).ready(function () {
    ...
    $("#btnRegistrarAlumno").click(function (event) {
        ...
        if (no_cuenta != "" && password != "" && nombre != "" && ap != "" && am != "" && sexo != "Seleccionar..."
            && telefono != 0 && email != "") {
            $.ajax({
                url: "./php/servidor.php",
                type: "GET",
                data: {
                    accion: accion, no_cuenta: no_cuenta, password: password, nombre: nombre, ap: ap, am: am,
    ...
    $("#btnRegistrarAsesor").click(function (event) {
        ...
        if (clave != "" && password != "" && nombre != "" && ap != "" && am != "" && sexo != "Seleccionar..."
            && telefono != 0 && email != "") {
            $.ajax({
                url: "./php/servidor.php",
                type: "GET",
                data: {
                    accion: accion, clave: clave, password: password, nombre: nombre, ap: ap, am: am,
    ...
    $("#btnRegistrarTrabajo").click(function (event) {
        //declaracion de variables
        var nombreTrabajo, tipo, nocuentaAlumnoTrabajo, claveasesorTrabajo, archivo, accion;
        //se recogen los datos de las cajas de texto
        archivo = $("#archivo").val();
        if (archivo != "") {
            nombreTrabajo = $("#txtNombreTrabajo").val();
            tipo = $("#txtTipoTrabajo").val();
            mes = $("#SelectMesTrabajo").val();
            año = $("#SelectAñoTrabajo").val();
            nocuentaAlumnoTrabajo = $("#txtCuentaAlumnoTrabajo").val();
            claveasesorTrabajo = $("#txtAsesorTrabajo").val();
            if (nombreTrabajo != "" && mes != "Seleccionar..." && año != "Seleccionar..." && tipo != "" && nocuentaAlumnoTrabajo != "" && claveasesorTrabajo != "") {
            ...
            alertify.alert("No se ha subido un archivo");
        }
    });
    ...
    $("#btnModalEditarAlumno").click(function (event) {
        ...
        if (no_cuentaM != null && nombreM != null && apM != null && amM != null && sexoM != "Seleccionar..."
            ...
            $.ajax({
                ...
                type: "GET",
                data: {
                    accion: accion, id_AlumnoM: id_AlumnoM, no_cuentaM: no_cuentaM, nombreM: nombreM, apM: apM, amM: amM,
    ...
    $("#btnModalEditarAsesor").click(function (event) {
        ...
        if (id_AsesorM != "" && claveM != "" && nombreM != "" && apM != "" && amM != "" && telefonoM != "" && emailM != "") {
            $.ajax({
                ...
                type: "GET",
                data: {
                    accion: accion, id_AsesorM: id_AsesorM, claveM: claveM, nombreM: nombreM, apM: apM, amM: amM,
    ...
    $("#btnModalEditarTrabajo").click(function (event) {
        ...
        archivo = $("#archivoModal").val();
        if (archivo != "") {
            alertify.confirm("¿Deseas reemplazar el archivo existente?", function (respuesta) {
                if (respuesta) {
                    ...
                    if (id_proyectoM != "" && nombreTrabajoM != "" && mesM != "" && añoM != "" && tipoM != "" && nocuentaAlumnoTrabajoM != "" && claveasesorTrabajoM != "") {
                        var parametros = new FormData($("#formulario-enviar-Modal")[0]);
                        ...
                        $.ajax({
                            ...
                            success: function (respuestaPHP) {
                                if (respuestaPHP == "1") {
                                    alert("Datos del Trabajo actualizados correctamente");
    ...
    function limpiarCamposTrabajos() {
        ...
        $("#SelectMesTrabajo").val("Seleccionar...");
        $("#SelectAñoTrabajo").val("Seleccionar...");
        $("#txtTipoTrabajo").val("");
    ...
    // mostrar nombre
    $("#txtCuentaAlumnoTrabajo").keyup(function(){
        digitos=$("#txtCuentaAlumnoTrabajo").val().length;
        if(digitos > 1){
            No_Cuenta=$("#txtCuentaAlumnoTrabajo").val();
            $.ajax({
                type:"GET",
                url:"./php/servidor.php",
                data:{accion:"buscaNoCuenta", No_Cuenta:No_Cuenta},
                    success:function(respuesta)
                    {
                        if(respuesta!=""){
                            $("#labelNoCuenta").html("<font color='green'><b>"+respuesta+"</b></font>");
                            $("#btnRegistrarTrabajo").show("100");
                        }
                        else{
                            $("#labelNoCuenta").html("<font color='red'><b>EL NÚMERO DE CUENTA NO EXISTE</b></b></font>");
                            $("#btnRegistrarTrabajo").hide("fast");
                        }
                    }
            });
        }
    ...
    // mostrar Asesor
    $("#txtAsesorTrabajo").keyup(function(){
        digitos=$("#txtAsesorTrabajo").val().length;
        if(digitos > 0){
            ClaveAsesor=$("#txtAsesorTrabajo").val();
            $.ajax({
                type:"GET",
                url:"./php/servidor.php",
                data:{accion:"buscaClaveAsesor", ClaveAsesor:ClaveAsesor},
                    success:function(respuesta)
                    {
                        if(respuesta!=""){
                            $("#labelAsesor").html("<font color='green'><b>"+respuesta+"</b></font>");
                            $("#btnRegistrarTrabajo").show("100");
                        }
                        else{
                            $("#labelAsesor").html("<font color='red'><b>LA CLAVE NO EXISTE</b></b></font>");
                            $("#btnRegistrarTrabajo").hide("fast");
                        }
                    }
            });
