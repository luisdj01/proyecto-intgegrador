<?php
    ...
    switch ($accion) {
        ...
        case 'RegistrarTrabajo':
            ...
            //se define la consulta sql a realizar
            $sql1="insert into trabajos values (0,'$nombreTrabajo','$mes','$año','$tipo','$nocuentaAlumnoTrabajo','$claveasesorTrabajo')";
            // Se ejecuta la consulta con la base de datos
        ...
        case 'buscaNoCuenta':
            $No_Cuenta=$_GET["No_Cuenta"];
            $sql="Select *from alumno where No_Cuenta='$No_Cuenta'";
            $resultado=$conexion->query($sql);
            $total=$resultado->num_rows;
            if ($total==1) {
                $fila=$resultado->fetch_array();
            ...
            break;
        case 'buscaClaveAsesor':
            $ClaveAsesor=$_GET["ClaveAsesor"];
            $sql="Select *from asesor where Clave='$ClaveAsesor'";
            $resultado=$conexion->query($sql);
            $total=$resultado->num_rows;
            if ($total==1) {
                $fila=$resultado->fetch_array();
